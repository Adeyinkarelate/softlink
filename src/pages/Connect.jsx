...
const errorMessage = () => toast.custom((t) => (
  <div className={`sm:max-w-sm  ${t.visible ? 'animate-enter' : 'animate-leave' }`}>

    <div className={`p-4 bg-red-600 border border-gray-200 rounded-xl shadow-sm`}>
        <div className="flex gap-x-4">          

          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="flex-shrink-0 w-8 h-auto text-white">
          ...
          <div>
            <button type="button" onClick={() => toast.dismiss(t.id)} className="inline-flex rounded-full p-2 text-white hover:bg-gray-700 focus:outline-none " data-hs-remove-element="#cookies-simple-icon-and-with-dismiss-button">
        ...
        <div id="segment-1" role="tabpanel" aria-labelledby="segment-item-1">
          <Formik
            ...
            onSubmit={(values, { setSubmitting, resetForm }) => {
              setTimeout(() => {
                const serviceID = "service_qrpcx0k";
                const templateID = "template_uuiki5p";
                const publicKey = "sgQ5SUoAhB-yeauhA";
                emailjs
          ...
          >
            {(formik) => (
              <form className="p-3" onSubmit={formik.handleSubmit}>
                ...
                <button
                  type="submit"
                  className="w-full py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-emerald-600 text-white hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition-all text-sm">
                  {formik.isSubmitting ? (
                    "Importing..."
                  ) : (
                    "Import"
